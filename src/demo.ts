...
export function makeDemo(): Session {
  ...
  return {
    ...
    tabs: entries.map(([id, title, url, opened, closed, openerId, accent, dark]) => ({
      ...
      openerId, windowId: id === 'vercel' ? '2' : '1', desktopId: 'Demo desktop', windowHistory: [{ windowId: id === 'vercel' ? '2' : '1', desktopId: 'Demo desktop', at: start + opened * 60000 }], active: id === 'react', focused: id === 'react', groupId: ['react', 'react-learn'].includes(id) ? 1 : null, groupTitle: ['react', 'react-learn'].includes(id) ? 'Learning React' : null, groupColor: ['react', 'react-learn'].includes(id) ? 'cyan' : null, groupCollapsed: false, groupHistory: [], thumbnail: demoThumbnail(id.startsWith('react') ? 'React' : id.charAt(0).toUpperCase() + id.slice(1), accent, dark),
